...
def my_recommender_paramsearch(rate_mat, test_mat, lr, with_reg):
    ...
    # learning_rate_arr = np.array([0.010, 0.0005, 0.0001, 0.00005])
    # max_iter_arr = np.array([200, 300, 500])
    # epsilon_arr = np.array([1e-5, 1e-6, 5e-6])
    reg_coef_arr = np.array([0.05, 0.02, 0.005, 0.0005])
    n_user, n_item = rate_mat.shape[0], rate_mat.shape[1]
    max_iter = 500
    epsilon = 1e-6
    learning_rate = 0.0003
    ...
    while (reg_coef_arr.size > 0):
        ...
        #     learning_rate = np.random.choice(learning_rate_arr, 1, replace=False)
        ...
        if reg_coef_arr.size > 0:
            reg_coef = np.random.choice(reg_coef_arr, 1, replace=False)
